...
def load_dense_baseline(path: Path) -> dict[str, Any] | None:
    if not path.exists():
        return None
...
def main() -> None:
    ...
    dense = load_dense_baseline(args.dense_baseline_report)
    if dense:
    ...
    with args.details_output.open("w", encoding="utf-8") as handle:
        for record in records:
            expected_ids = {chunk["chunk_id"] for chunk in record["expected_chunks"]}
            rrf_rows = [
                {**point_row(point, rank), "is_expected": point["payload"]["chunk_id"] in expected_ids}
